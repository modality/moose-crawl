...
*/ function isLVar(x) {
    ...
    function pprint(x) {
        if (x && x.constructor === ImmutableMap) x.pprint();
        else console.log(x);
    }
    const dot = function() {};
    /**
 ...
 */ function unify(x, y, sMap) {
        x = walk(x, sMap);
        y = walk(y, sMap);
        const xIsLVar = isLVar(x), yIsLVar = isLVar(y);
        if (x === y) return sMap;
        else if (xIsLVar) return sMap.set(x, y);
        else if (yIsLVar) return sMap.set(y, x);
        else if (isArray(x) && isArray(y)) return unifyArray(x, y, sMap);
        else return null;
    }
    // x and y are arrays
    function unifyArray(x, y, sMap) {
        ...
        else if (y[0] === dot) return unify(y[1], x, sMap);
        else if (x.length && !y.length || !x.length && y.length) return null;
 ...
 */ function and(...clauses) {
        return function*(sMap1) {
            function* helper(offset, sMap) {
                if (offset === clauses.length) return;
                let clause = clauses[offset];
                if (clause.constructor.name !== "GeneratorFunction") clause = clause();
                let gen = clause(sMap);
                while(true){
                    let res = gen.next(), sMap = res.value;
                    if (res.done) break;
                    if (sMap) {
                        if (offset === clauses.length - 1) yield sMap;
                        else yield* helper(offset + 1, sMap);
                    } else yield null // failed
                    ;
                }
            }
            yield* helper(0, sMap1);
        };
    }
    /**
 * or
 */ function or(...clauses) {
        let count = false, startOffst = 0;
        solNum = 0;
        if (typeof clauses[0] === "number") {
            count = clauses[0];
            startOffst = 1;
        }
        return function*(sMap2) {
            function* helper(offset, sMap, solNum) {
                ...
                while(true){
                    const res = gen.next(), sMap = res.value;
                    if (res.done) break;
                    if (sMap) {
                        yield sMap;
                        solNum++;
            ...
            yield* helper(startOffst, sMap2, solNum);
        };
    }
    /**
 * eq
 ...
 */ function run(num, vars, goal) {
        ...
        if (goal.constructor.name !== "GeneratorFunction") goal = goal();
        if (!(vars instanceof Array)) vars = [
            vars
        ];
        const results = [];
        let sMap = new ImmutableMap(), gen = goal(sMap);
        while(num){
            const res = gen.next(), sMap = res.value;
            if (res.done) break;
            if (sMap) {
    ...
    function membero(x, arr) {
        return or((first = lvar())=>and(firsto(first, arr), eq(first, x)), (rest = lvar())=>and(resto(rest, arr), membero(x, rest)));
    }
    function appendo(seq1, seq2, out) {
        return or(and(emptyo(seq1), eq(seq2, out)), (first = lvar(), rest = lvar(), rec = lvar())=>and(conso(first, rest, seq1), conso(first, rec, out), appendo(rest, seq2, rec)));
    }
    /*
 ...
 */ // a + b = c
    function add(a, b, c) {
        return function*(sMap) {
            let numOfLVars = 0, lvar_ = null;
            a = walk(a, sMap);
            b = walk(b, sMap);
            c = walk(c, sMap);
            const aIsLVar = isLVar(a), bIsLVar = isLVar(b), cIsLVar = isLVar(c);
            if (aIsLVar) {
                lvar_ = a;
                numOfLVars++;
            }
            ...
            if (numOfLVars === 0) {
                ...
                if (lvar_ === a) {
                    if (typeof c === "number" && typeof b === "number") yield* eq(a, c - b)(sMap);
                    else yield null;
                } else if (lvar_ === b) {
                    if (typeof c === "number" && typeof a === "number") yield* eq(b, c - a)(sMap);
                    else yield null;
                } else if (typeof a === "number" && typeof b === "number") yield* eq(c, a + b)(sMap);
                else yield null;
            } else yield null;
        };
    }
 ...
 */ function facts(...facs) {
        return function(...args) {
            return or.apply(null, facs.map((fac)=>and.apply(null, fac.map((facArg, i)=>eq(facArg, args[i])))));
        };
    }
    function anyo(goal) {
...
//# sourceMappingURL=index.29e09695.js.map
